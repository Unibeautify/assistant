...
export class DebugReport {
  ...
  get atomBeautifyVersion(): string {
    return (
      this.parse("[*][*]Atom Beautify Version[*][*]: (.+)\n", "unknown") || ""
    );
  }

  get filePath(): string {
    return (
      this.parse("[*][*]Original File Path[*][*]: `(.+)`", "unknown") || ""
    );
  }

  get grammar(): string {
    return (
      this.parse("[*][*]Original File Grammar[*][*]: (.+)\n", "unknown") || ""
    );
  }

  get language(): string {
    return (
      this.parse("[*][*]Original File Language[*][*]: (.+)\n", "unknown") || ""
    );
  }

  get languageNamespace(): string {
    return (
      this.parse("[*][*]Language namespace[*][*]: (.+)\n", "unknown") || ""
    );
  }

  get beautifiers(): string {
    return (
      this.parse("[*][*]Supported Beautifiers[*][*]: (.+)\n", "unknown") || ""
    );
  }

  get beautifier(): string {
    return (
      this.parse("[*][*]Selected Beautifier[*][*]: (.+)\n", "unknown") || ""
    );
  }
  ...
  get error(): BeautifyError | undefined {
    let failedExecutable: string | undefined;
    if (
      (failedExecutable = DebugReport.parse(
        this.logs,
        "Error loading executables.+ Could not find '(.+)'."
      ))
    ) {
      let versionText: string | undefined;
      if (
        (versionText = DebugReport.parse(
          this.logs,
